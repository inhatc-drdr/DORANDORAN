...
app.post("/server/invent", (req, res) => {
  ...
  let sql =
    "SELECT count(*) as count FROM srvuser WHERE srv_id=? " +
  ...
  DB(sql, params).then(function (result) {
    if (!result.state) {
      ...
      if (count) {
        ...
        // insert db
        sql =
          "INSERT INTO srvuser (srv_id, user_id) " +
    ...
    if (!result.state) {
      ...
      if (!result.rows[0]) {
        ...
        if (user_id == admin_id) {
          // insert db
          sql =
            "INSERT INTO notice (srv_id, user_id, notice_name, notice_memo)" +
            " VALUES(?,?,?,?)";
...
app.post("/server/notice/list", (req, res) => {
  ...
  // check if the user is the administrator of the server
  let sql =
    "SELECT notice_id, notice_name, notice_write " +
    "FROM notice n " +
